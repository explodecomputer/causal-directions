...
sfig_nums(
	"unmeasured",
	caption=paste0("Graph representing the unmeasured confounding parameters that will lead to the Steiger test returning the wrong causal direction. Columns of boxes represent different signed values of the observational variance explained between $x$ and $y$ ($R^2_{xy}$)."),
	display=FALSE
)
...
where $\epsilon_{m*} \sim N(0, \sigma^2_{m*})$, $\alpha_{mx}$ and $\beta_{mx}$ are parameters that represent non-differential measurement error into the exposure variable $x$, and $\alpha_{my}$ and $\beta_{my}$ are parameters for non-differential measurement error in the outcome $y$. The simulations in `r cfig("bdmr")` are similar except we also simulate a second SNP which has a direct influence on $y$. MR can therefore be performed using the first SNP for estimating the effect of $x$ on $y$, and using the second SNP for estimating the effect of $y$ on $x$.


We have formulated the the non-causal model as:
...
Here the $u$ variable mediates the influence of $g$ on $x$ and $g$ on $y$. The effects of $u$ on $x$ and $y$ are identical, therefore this simply attenuates the variance explained by $g$ on $x$ and $y$. All $\alpha$ values were set to 0, and $\beta$ values set to 1. Normally distribted values of $\epsilon_*$ were generated such that 
...
The bi-directional MR approach does offer a solution to this problem (`r cfig("bdmr")`)
...
### Unmeasured confounding in the Steiger test

Similarly, we can investigate the extent to which unmeasured confounding will lead to the wrong causal direction between $x$ and $y$ using the Steiger test, evaluating the liability for the inequality $cor(g,x)^2 > cor(g,y)^2$ being incorrect. After some algebra
...
`r csfig("unmeasured")` shows the relationship between the magnitude of the correlations between $x$, $y$ and the confounder $u$. We note that in most cases where the observational absolute $R^2_{xy}$ is less than 0.2, unmeasured confounding will not incur the wrong causal direction in the Steiger test.
