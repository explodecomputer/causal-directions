...
We used the Steiger test to infer the direction of causality between DNA methylation levels and gene expression levels. To do this we obtained a list of `r nrow(qtl)` gene expression - DNA methylation associations as reported in Shakhbazov et al [-@Shakhbazov2016]. These were filtered to be located on the same chromosome, have robost correlations after correcting for multiple testing, and to share a SNP that had a robust cis-acting effect on both the DNA methylation probe and the gene expression probe. Using the reported summary data we used the Steiger test of two independent correlations to identify which direction of causality was most likely for each association. We found that the causal direction commonly goes in both directions (`r ctab("shakhtab")`) but methylation levels were more likely to influence expression levels than vice versa (probability of expression causing methylation = `r shakhtest3$p.value` (95% CI `r shakhtest3$conf.int[1]` - `r shakhtest3$conf.int[2]`) amongst `r sum(shakhbazov$dir_p < 0.05)` associations with $p_{Steiger} < 0.05$). 
