...
& = cov(\beta_g g + \beta_{ux} u + \epsilon_g, \beta_x x + \beta_{uy} u + \epsilon_x) / var(x) \\
& = cov(\beta_g g + \beta_{ux} u + \epsilon_g, \beta_x x + \beta_{uy} u + \epsilon_x) / var(x) \\
& = cov(\beta_g g + \beta_{ux} u + \epsilon_g, \beta_x \beta_g g + \beta_x \beta_{ux} u + \beta_x \epsilon_g + \beta_{uy} u + \epsilon_x) / var(x) \\
& = \frac{\beta_g^2 \beta_x var(g) + \beta_{ux}^2 \beta_x var(u) + \beta_x var(\epsilon_g)} {\beta_g^2 var(g) + \beta_{ux}^2 var(u) + var(\epsilon_g)}
\end{aligned}
$$

From this it is clear that $\beta_x$ and $\hat{\beta}_x$ will differ when both $\beta_{uy}$ and $\beta_{ux}$ are both non-zero. In terms of mediation, where we attempt to test if $g$ associates with $y$ after adjusting $y$ for $x$, such that
...
Should any amount of unmeasured confounding exist, therefore, there will remain an association between $g$ and the $y|x$, which will introduce errors in inferring causal directions.

### Unmeasured confounding in the Steiger test

Similarly, we can investigate the extent to which unmeasured confounding will lead to the wrong causal direction between $x$ and $y$ using the Steiger test, evaluating the liability for the inequality $cor(g,x)^2 > cor(g,y)^2$ being incorrect.
