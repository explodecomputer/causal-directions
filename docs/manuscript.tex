...
demonstrate that when there is uncertainty about the nature of the
genetic association existing methods that are used for causal inference
can be liable to give the wrong direction of causality. We introduce a
new method that is more flexible and robust to problems such as
measurement error, and apply it to evaluate the causal relationships
between DNA methylation levels and gene expression. Our results show
that DNA methylation is more likely to be the causal factor, but that
systematic differences in measurement error between the platforms could
...
direction between the two phenotypes.
...
outcome (29), and in order to fully abrogate this association one must
...
detail, we show that attempting to adjust for mediating variables to
...
should be practiced in causal inference (36).
...
limits on sensitivity of measuring equipment, or due to phenotypes being
imprecisely defined. The same model of measurement error can be applied
to the outcome variable \(y\).
...
power in MR but will not incur bias (3,13,30). We also assume that
...
(26,28)). Here the Steiger test of two independent correlations can be
applied where.
...
other circumstances also. First, some levels of horizontal pleiotropy,
where the SNP influences the outcome through some pathway other than the
...
bi-directional MR can be used to orient causal directions, this requires
knowledge of a valid instrument for each trait, and we were motivated to
develop the MR Steiger method that could operate on summary data to
orient the direction of causality using the same conditions as the CIT,
where the underlying biology of a single SNP is not fully understood. We
go on to explore the scenarios in which the method is likely to return
the correct or incorrect causal directions.
...
reduces as measurement error in the exposure increases faster than that
of the MR Steiger method.
...
rise to problems with the MR Steiger approach (Appendix 3). Though the
relationship between unmeasured confounding and causal orientation is
complex (Supplementary Figure 1: Graph representing the unmeasured
...
(\(R^2_{xy}\)).), we note that when the magnitude of the observational
variance explained between the exposure and the outcome is below 0.2 the
MR Steiger method is unlikely to return the incorrect causal direction.
...
influencing different lipid fractions, for example (51,52).
...
the correlations between \(x\), \(y\) and the confounder \(u\). We note
that in most cases where the observational absolute \(R^2_{xy}\) is less
