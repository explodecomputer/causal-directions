...
causal trait can pose lead existing causal inference methods to give the
...
(\(R^2_{xy}\)).). Based on the range of parameter values that we
...
values explored, where the observational \(\hat{R}^2_{xy}\) is less than
0.2, unmeasured confounding will not incur the wrong causal direction in
the MR Steiger test.
